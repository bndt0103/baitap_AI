...
- **Sensorless Search (Môi trường không nhìn thấy)**: Robot hoàn toàn không biết vị trí hiện tại của mình do không có cảm biến. Thuật toán hoạt động bằng cách duy trì một "Belief State" (tập hợp mọi trạng thái có thể) và tìm ra một chuỗi hành động ép buộc (coerce) để đảm bảo hoàn thành mục tiêu dọn sạch rác bất kể vị trí ban đầu là ở đâu.
- **Partially Observable Search (Môi trường nhìn thấy một phần)**: Robot bị giới hạn tầm nhìn, chỉ nhận được dữ liệu cục bộ từ cảm biến (như có rác ở ô hiện tại không, có tường ở 4 hướng không). Thuật toán thực hiện chu trình dự đoán (predict) và cập nhật (update) Belief State liên tục dựa trên phản hồi của cảm biến sau mỗi bước đi để xác định vị trí thực tế và ra quyết định dọn dẹp.
...
- **Expectimax**: Mở rộng từ Minimax để đối phó với môi trường hoặc đối thủ hành động mang tính ngẫu nhiên, không hoàn toàn tối ưu. Thay vì cho rằng đối thủ (node Min) luôn chọn nước đi gây thiệt hại nhất cho ta, Expectimax sử dụng "Chance Node" (Nút cơ hội) để tính giá trị trung bình kỳ vọng (Expected value) dựa trên xác suất các hành động của đối thủ. Điều này giúp AI linh hoạt và chủ động tận dụng sai lầm của đối thủ thay vì phòng thủ quá tiêu cực.
